...
module FactOutput = 
    ...
    type Clause = 
        { FactName: Identifier
          Values : PrologSyntax.Value list }
        member v.Format = 
            prologFact (simpleAtom v.FactName) 
                        (List.map (fun (x:PrologSyntax.Value) -> x.Format()) v.Values)

    /// Note - if we parsed/validated the signature we could save the user
    /// having to specify name and arity.
    type IFactHelper<'a> = 
        abstract Signature : string
        abstract ClauseBody : 'a -> Option<PrologSyntax.Value list>
    ...
    let makeFactSet (helper:IFactHelper<'a>) (items:seq<'a>) : FactSet = 
        ...
        let makeClause (item:'a) : Option<Clause>  = 
            let make1 (values:PrologSyntax.Value list) : Clause = 
